...
import type { RelatedVarietiesSection } from "../types";
...
interface Props {
  section: RelatedVarietiesSection;
}

export function RelatedVarietiesSectionRenderer({ section }: Props) {
    ...
    <SectionCard title={section.title} icon={section.icon}>
      {section.items.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          No varieties found for this species
        </p>
      ) : (
        <>
          <Table>
            ...
            <TableBody>
              {section.items.map((item) => (
                <TableRow
                  ...
                  onClick={() =>
                    navigate(`/inventory/products/varieties/${item.id}`)
                  }
                >
