...
import type { GrowthCurveDataPoint } from "@/shared/types";
import {
    CartesianGrid,
    Line,
    LineChart,
...
const GrowthChart = ({
  ...
  const chartData = logs.map((log) => ({
    week: `W${log.week}`,
    weekNum: log.week,
    alive: log.alive_count,
    total: log.seedling_count,
    survival: log.survival_rate,
  ...
  return (
    <div className={className}>
      {title && (
        <h4 className="text-sm font-medium tracking-wider text-muted-foreground mb-4">
          {title}
        </h4>
      )}
        ...
        >
          ...
          {showTarget && (
            <ReferenceLine
              ...
              strokeDasharray="8 4"
              label={{
                value: `Target: ${showTarget.toLocaleString()}`,
                position: "right",
                fontSize: 11,
                fontWeight: 700,
                fill: "hsl(0, 72%, 51%)",
      ...
      <div className="flex items-center gap-6 mt-3 text-xs font-medium">
        ...
        <div className="flex items-center gap-2">
          <div
            className="w-4 h-0.5 bg-[hsl(175,65%,35%)] border-dashed"
        ...
        {showTarget && (
          <div className="flex items-center gap-2">
            <div
              className="w-4 h-0.5 bg-[hsl(0,72%,51%)]"
              style={{ borderTopWidth: 2, borderTopStyle: "dashed" }}
            />
