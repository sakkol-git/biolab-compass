    ...
    private function searchableModels(): array
    {
        return [
            ...
            [
                ...
                'type' => 'experiment',
                'label' => 'Experiments',
                'fields' => ['title', 'experiment_code'],
                'display' => fn ($m) => $m->title,
                'subtitle' => fn ($m) => $m->experiment_code,
                'url' => fn ($m) => "/research/experiments/{$m->id}",
            ],
            [
                'model' => Protocol::class,
                'type' => 'protocol',
                'label' => 'Protocols',
                'fields' => ['title', 'protocol_code'],
                'display' => fn ($m) => $m->title,
                'subtitle' => fn ($m) => $m->protocol_code,
                'url' => fn ($m) => "/research/protocols/{$m->id}",
            ],
            [
                'model' => Client::class,
                'type' => 'client',
                'label' => 'Clients',
                'fields' => ['name', 'client_code', 'email'],
                'display' => fn ($m) => $m->name,
                'subtitle' => fn ($m) => $m->client_code,
                'url' => fn ($m) => "/business/clients/{$m->id}",
            ],
            [
                'model' => Contract::class,
                'type' => 'contract',
                'label' => 'Contracts',
                'fields' => ['contract_number', 'title'],
                'display' => fn ($m) => $m->title ?? $m->contract_number,
                'subtitle' => fn ($m) => $m->contract_number,
                'url' => fn ($m) => "/business/contracts/{$m->id}",
            ],
        ];
    }
