...
import { formatCurrency, formatNumber } from "@/shared/lib/calculator";
...
import type { SpeciesGrowthProfile } from "@/features/research";
...
const InputSection = ({ vm }: { vm: VM }) => (
  <div className="bg-card rounded-xl p-6 border border-border/60 shadow-md">
    ...
    {vm.selectedProfile && (
      <SpeciesProfileSummary profile={vm.selectedProfile} />
    )}
  </div>
);
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      {lowConfidence && selectedProfile && (
        <LowConfidenceAlert
          completedExperiments={selectedProfile.completedExperiments}
        />
      )}
  ...
  <div className="bg-amber-50 border border-amber-300 rounded-lg p-4 flex items-start gap-3 shadow-sm">
    ...
    <div>
      ...
      <p className="text-xs text-amber-700 mt-0.5">
        Only {completedExperiments} completed experiment(s) for this species.
        Minimum 3 recommended for accurate forecasting.
      </p>
    </div>
  </div>
);
  ...
  const cards = [
    {
      icon: Sprout,
      label: "Initial Stock",
      value: formatNumber(forecast.recommendedInitialStock),
      sub: "mother plants needed",
    },
    {
      icon: Clock,
      label: "Est. Time",
      value: (
        <>
          {forecast.estimatedWeeks} <span className="text-base">wks</span>
        </>
      ),
      sub: `90% CI: ${forecast.confidenceLowerWeeks}–${forecast.confidenceUpperWeeks} weeks`,
    },
    {
      icon: Building2,
      label: "Greenhouses",
      value: forecast.resourceRequirements.greenhouses,
      sub: "facility units required",
    },
    {
      icon: DollarSign,
      label: "Est. Cost",
      value: formatCurrency(forecast.resourceRequirements.estimatedCost),
      sub: "production cost",
    },
  ];
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
      {cards.map((c) => {
        ...
        return (
          <div
            key={c.label}
            className="bg-card rounded-xl p-4 border border-border/60 shadow-md"
          >
            <div className="flex items-center gap-2 mb-2">
              <Icon className="h-4 w-4 text-muted-foreground/50" />
              <span className="text-xs font-normal tracking-wider text-muted-foreground/70">
                {c.label}
              </span>
            </div>
...
const TimelineChart = ({ forecast }: { forecast: ProductionForecast }) => (
  <div className="bg-card rounded-xl p-5 border border-border/60 shadow-md">
    ...
    <ResponsiveContainer width="100%" height={300}>
      <LineChart
        data={forecast.weeklyMilestones}
...
const ForecastSummary = ({ forecast }: { forecast: ProductionForecast }) => {
  const entries = [
    ...
    {
      label: "Multiplication Rate",
      value: `${forecast.estimatedMultiplicationRate}×`,
    },
    {
      label: "Labor Hours",
      value: `${forecast.resourceRequirements.laborHours} hrs`,
    },
  ];

  return (
    <div className="bg-card rounded-xl p-5 border border-border/60 shadow-md">
      <h4 className="text-sm font-normal tracking-wider text-muted-foreground/70 mb-3">
        Forecast Summary
      </h4>
